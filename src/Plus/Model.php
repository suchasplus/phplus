...
class Plus_Model
{
     ...
     *
     * @param mixed $name
     * @return Plus_Com_Cache
     */
    public function cache($name = '_cache')
    {
        return Plus_Com::cache($name);
    }
    ...
    public function db($name = null)
    {
        ...
        if ($db = Plus::reg($name)) return $db;

        $config = (array)Plus::config($name) + array('adapter' => 'Mysql', 'params' => array());
        $db = Plus_Com_Db::factory($config);
        Plus::reg($name, $db);
        return $db;
    }
    ...
    {
        null === $dir && $dir = Plus::config('_modelsHome');
