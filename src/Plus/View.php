...
class Plus_View
{
     ...
     * @param string $name
     * @param array $data
     * @return Plus_Com_Widget
     */
    public function widget($name, $data = null)
    {
        if (empty($this->_widgetsHome) && $widgetsHome = Plus::config('_widgetsHome')) {
            $this->_widgetsHome = $widgetsHome;
        }

        $class = ucfirst($name) . 'Widget';

        if (!Plus::loadClass($class, $this->_widgetsHome)) {
            throw new Plus_Exception("Can not find widget:{$class}");
        }
